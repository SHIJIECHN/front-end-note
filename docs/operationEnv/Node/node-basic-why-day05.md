...
1. JavaScript是单线程的，但是JavaScript线程有自己的容器进程：浏览器或Node。
...
1. js引擎线程（解释和执行js代码，JS内核-V8引擎，js引擎用来解释执行js代码）
2. GUI线程（绘制用户界面，与js主线程是互斥的）
3. http网路请求线程（处理用户的get、post等请求等，返回结果后将回调函数推入到任务队列）
4. 定时器触发线程（setTimeout、setInterval、等待时间结束后把执行函数推入到任务队列中）
5. 浏览器事件处理线程（将click、mouse等交互事件发生后将这些事件放入任务队列中）