...
#### 通过import加载一个模块，是不可以将其放在逻辑代码中的。如果希望动态的加载某一个模块，可以使用import函数根据模块路径来动态加载。
```js
// 错误写法
if(true){
...
4. 在一个模块中，只能有一个默认岛主（default export）
